...
// Removed Component import for headless test

// Import task styles
// Removed CSS import for headless test
...
function safeLiftListItem(editor: Editor, itemDepth: number, itemType: string): boolean {
    ...
    return editor.chain()
        .command(({ tr: chainTr }) => {
            ...
            // 💡 关键加固：在 liftListItem 前先将选区重定位，确保 lift 命令 100% 执行成功
            const targetPos = Math.min(mappedStart + relativeOffset, chainTr.doc.content.size);
...
export const MixedTaskItem = TaskItem.extend({
    ...
    addNodeView() {
        return () => { return { dom: document.createElement('li') }; };
    },
});
