import React, { useState, useEffect, useRef } from 'react';
import { useTranslation } from 'react-i18next';
import { RELATION_GROUPS } from '@/shared/constants/relations';
import { getParentPath, getBasename } from '@/shared/utils/pathUtils';

export interface NewNodeInputPopupProps {
    position: { x: number; y: number };
    initialFolder: string;
    folders: string[];
    showFolderSelect: boolean;
    showRelation: boolean;
    onSubmit: (title: string, folder: string, relation?: string) => void;
    onCancel: () => void;
}

export function NewNodeInputPopup({ position, initialFolder, folders, showFolderSelect, showRelation, onSubmit, onCancel }: NewNodeInputPopupProps) {
    const [title, setTitle] = useState('');
    const [folder, setFolder] = useState(initialFolder || '00_Inbox');
    const [relation, setRelation] = useState('related');
    const [isFolderOpen, setIsFolderOpen] = useState(false);
    const [isRelationOpen, setIsRelationOpen] = useState(false);
    const [browsePath, setBrowsePath] = useState<string | null>(null);
    const [activeRelationGroup, setActiveRelationGroup] = useState<string | null>(null);
    const inputRef = useRef<HTMLInputElement>(null);
    const { t } = useTranslation();
    
    // Ensure if we open the folder select, we start browsePath at the current folder's parent (or root)
    useEffect(() => {
        if (isFolderOpen && folder.includes('/')) {
            const parent = getParentPath(folder);
            if (parent !== null) setBrowsePath(parent);
        }
    }, [isFolderOpen, folder]);

    const getVisibleFolders = () => {
        if (!browsePath) {
            return folders.filter(f => !f.includes('/'));
        }
        const prefix = browsePath + '/';
        return folders.filter(f => f.startsWith(prefix) && f.substring(prefix.length).indexOf('/') === -1);
    };

    const hasChildren = (folderPath: string) => {
        const prefix = folderPath + '/';
        return folders.some(f => f.startsWith(prefix));
    };

    useEffect(() => {
        inputRef.current?.focus();
    }, []);

    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === 'Enter' && title.trim()) {
            onSubmit(title.trim(), folder, showRelation ? relation : undefined);
        } else if (e.key === 'Escape') {
            onCancel();
        }
    };

    return (
        <>
            <div 
                className="fixed inset-0 z-90" 
                onClick={(e) => { e.stopPropagation(); onCancel(); }}
                onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); onCancel(); }}
            />
            <div
                className="fixed z-100 w-64 bg-white/95 dark:bg-zinc-800/95 backdrop-blur-xl 
                           border border-zinc-200/50 dark:border-zinc-700/50 rounded-xl shadow-2xl p-3.5 
                           animate-in fade-in zoom-in-95 font-sans"
                style={{ left: position.x, top: position.y }}
                onContextMenu={(e) => e.stopPropagation()}
            >
            <div className="text-[10px] font-semibold tracking-wider text-zinc-400 dark:text-zinc-500 uppercase mb-3 px-0.5">
                {t('graph.new_note', '创建新笔记')}
            </div>
            
            <input
                ref={inputRef}
                type="text"
                value={title}
                onChange={e => setTitle(e.target.value)}
                onKeyDown={handleKeyDown}
                placeholder={t('graph.note_title_placeholder', '输入笔记标题...')}
                className="w-full px-2 py-1.5 text-[13px] border-b border-zinc-300 dark:border-zinc-600
                           bg-transparent focus:outline-none focus:border-blue-500 
                           text-zinc-800 dark:text-zinc-200 mb-4 transition-colors font-medium placeholder:font-normal"
            />

            {showFolderSelect && (
                <div className="relative mb-3">
                    <button 
                        onClick={() => { setIsFolderOpen(!isFolderOpen); setIsRelationOpen(false); }}
                        className="flex items-center justify-between w-full px-2.5 py-1.5 text-xs bg-zinc-100/50 dark:bg-zinc-700/50 hover:bg-zinc-100 dark:hover:bg-zinc-700 border border-zinc-200/50 dark:border-zinc-600/50 rounded-md text-zinc-700 dark:text-zinc-300 transition-colors"
                    >
                        <div className="flex items-center gap-2 truncate">
                            <span className="text-[10px] bg-zinc-200/50 dark:bg-zinc-800 rounded px-1 text-zinc-500">Dir</span>
                            <span className="truncate">{folder}</span>
                        </div>
                        <span className="text-zinc-400 text-[10px]">▼</span>
                    </button>
                    {isFolderOpen && (
                        <div className="absolute top-full left-0 mt-1 w-full max-h-[160px] flex flex-col bg-white/95 dark:bg-zinc-800/95 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-xl rounded-lg z-50 p-1 animate-in fade-in zoom-in-95">
                            {browsePath && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        setBrowsePath(getParentPath(browsePath));
                                    }}
                                    className="flex shrink-0 items-center w-full px-2 py-1.5 text-left text-xs rounded-md text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors mb-0.5 sticky top-0 z-10 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md"
                                >
                                    <span className="mr-1 mt-px">‹</span>
                                    <span className="truncate flex-1">{t('common.back', '返回')} .../{getBasename(browsePath)}</span>
                                </button>
                            )}
                            <div className="overflow-y-auto flex-1">
                                {getVisibleFolders().map(f => {
                                    const name = getBasename(f) || f;
                                    const isSelected = f === folder;
                                    const canExpand = hasChildren(f);
                                    return (
                                        <div key={f} className={`flex items-stretch w-full rounded-md transition-colors mb-0.5 ${isSelected ? 'bg-blue-500 text-white dark:bg-blue-600' : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}>
                                            <button 
                                                onClick={() => { setFolder(f); setIsFolderOpen(false); }}
                                                className="flex-1 flex items-center px-2 py-1.5 text-left text-xs min-w-0"
                                            >
                                                <span className="mr-1.5 text-[10px] w-3 flex justify-center">{isSelected ? '✓' : ''}</span>
                                                <span className="truncate">{name}</span>
                                            </button>
                                            {canExpand && (
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); setBrowsePath(f); }}
                                                    className={`px-1.5 flex items-center text-[9px] opacity-70 hover:opacity-100 ${isSelected ? 'text-white hover:bg-white/20' : 'hover:bg-zinc-200 dark:hover:bg-zinc-600'} rounded-r-md transition-colors`}
                                                >
                                                    ❯
                                                </button>
                                            )}
                                        </div>
                                    );
                                })}
                            </div>
                        </div>
                    )}
                </div>
            )}

            {!showFolderSelect && (
                <div className="flex items-center gap-2 px-2 py-1.5 text-xs text-zinc-500 dark:text-zinc-400 mb-3 bg-zinc-100/50 dark:bg-zinc-800/50 border border-zinc-200/50 dark:border-zinc-700/50 rounded-md">
                    <span className="text-[10px] bg-zinc-200/80 dark:bg-zinc-700 rounded px-1">Dir</span> 
                    <span className="truncate">{initialFolder}</span>
                </div>
            )}

            {showRelation && (
                <div className="relative mb-3">
                    <button 
                        onClick={() => { setIsRelationOpen(!isRelationOpen); setIsFolderOpen(false); }}
                        className="flex items-center justify-between w-full px-2.5 py-1.5 text-xs bg-zinc-100/50 dark:bg-zinc-700/50 hover:bg-zinc-100 dark:hover:bg-zinc-700 border border-zinc-200/50 dark:border-zinc-600/50 rounded-md text-zinc-700 dark:text-zinc-300 transition-colors"
                    >
                        <div className="flex items-center gap-2 truncate">
                            <span className="text-[10px] bg-zinc-200/50 dark:bg-zinc-800 rounded px-1 text-zinc-500">Link</span>
                            <span className="truncate">{t(`relations.${relation}`, relation)}</span>
                        </div>
                        <span className="text-zinc-400 text-[10px]">▼</span>
                    </button>
                    {isRelationOpen && (
                        <div className="absolute top-full left-0 mt-1 w-full max-h-[160px] overflow-y-auto bg-white/95 dark:bg-zinc-800/95 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-xl rounded-lg z-50 p-1 animate-in fade-in zoom-in-95 custom-scrollbar">
                            {activeRelationGroup === null ? (
                                <div className="flex flex-col gap-0.5">
                                    {RELATION_GROUPS.map(g => (
                                        <div key={g.labelKey} className="flex items-stretch w-full rounded-md transition-colors text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700">
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setActiveRelationGroup(g.labelKey); }}
                                                className="flex-1 flex items-center px-2 py-1.5 text-left text-xs min-w-0"
                                            >
                                                <span className="mr-1.5 text-[10px] w-3 flex justify-center"></span>
                                                <span className="truncate">{t(g.labelKey, g.label)}</span>
                                            </button>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setActiveRelationGroup(g.labelKey); }}
                                                className="px-1.5 flex items-center text-[9px] opacity-70 hover:opacity-100 hover:bg-zinc-200 dark:hover:bg-zinc-600 rounded-r-md transition-colors"
                                            >
                                                ❯
                                            </button>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setActiveRelationGroup(null); }}
                                        className="flex shrink-0 items-center w-full px-2 py-1.5 text-left text-xs rounded-md text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors mb-0.5 sticky top-0 z-10 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md"
                                    >
                                        <span className="mr-1 mt-px">‹</span>
                                        <span className="truncate flex-1">{t('common.back_to_group', '返回 分组')} / {t(RELATION_GROUPS.find(g => g.labelKey === activeRelationGroup)?.labelKey || '', RELATION_GROUPS.find(g => g.labelKey === activeRelationGroup)?.label || '')}</span>
                                    </button>
                                    <div className="overflow-y-auto flex-1 flex flex-col gap-0.5">
                                        {RELATION_GROUPS.find(g => g.labelKey === activeRelationGroup)?.keys.map(r => (
                                            <button 
                                                key={r}
                                                onClick={() => { setRelation(r); setIsRelationOpen(false); setActiveRelationGroup(null); }}
                                                className={`flex items-center w-full px-2 py-1.5 text-left text-xs rounded-md transition-colors ${r === relation ? 'bg-blue-500 text-white dark:bg-blue-600' : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
                                            >
                                                <span className="mr-1.5 text-[10px] w-3 flex justify-center">{r === relation ? '✓' : ''}</span>
                                                <span className="truncate flex-1">{t(`relations.${r}`, r)}</span>
                                            </button>
                                        ))}
                                    </div>
                                </>
                            )}
                        </div>
                    )}
                </div>
            )}

            <div className="flex gap-2 mt-1">
                <button
                    onClick={() => title.trim() && onSubmit(title.trim(), folder, showRelation ? relation : undefined)}
                    className="flex-1 px-3 py-1.5 text-xs font-medium bg-blue-500 text-white rounded-md hover:bg-blue-600 shadow-sm transition-colors"
                >
                    {t('common.create', '创建')}
                </button>
                <button
                    onClick={onCancel}
                    className="flex-1 px-3 py-1.5 text-xs font-medium bg-zinc-100 dark:bg-zinc-700 rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-600 text-zinc-700 dark:text-zinc-300 transition-colors"
                >
                    {t('common.cancel', '取消')}
                </button>
            </div>
        </div>
        </>
    );
}
